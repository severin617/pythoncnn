...
#data = x_train[:60000]
#labels = np.asarray(np.reshape(y_train[:60000],(60000,1)), dtype=np.int32)
...
#val_data = mnist.test.images  # Returns np.array
#val_labels = np.asarray(mnist.test.labels, dtype=np.int32)
#val_data = x_test # np.random.random((100, 32))
#val_labels = y_test # np.random.random((100,1))


if training:
	sess = tf.InteractiveSession()
	sess.run(tf.global_variables_initializer()) # deprecated: initialize_all_variables())
	history = model.fit(data, labels, epochs=1, batch_size=global_batch_size)
	model.save_weights('./saved_models/test_bias')
else:
	sess = tf.InteractiveSession()
	model.load_weights('./saved_models/test_bias')
...
evaluated_gradients = sess.run(gradients,feed_dict={model.inputs[0]:data[:global_batch_size]})
...
def finite_diff():
	...
	for i in range(n):
		...
		f_true[i]=sess.run(loss,feed_dict={model.inputs[0]:data})
...
def hessian_single_eval(begin=0, ending=aggregated_layer_weigths[-1]):
...
f=open("./data/mlp-mnist01s"+str(start)+"e"+str(end),"wa")
hessian_single_eval(start,end)
f.close()

#appendedList=hessian_single_eval()
#HH=sess.run(appendedList, feed_dict={model.input:data})
