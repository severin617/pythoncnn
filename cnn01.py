...
model.add(tf.keras.layers.Conv2D(1,input_shape=(28,28,1), kernel_size=(4, 4),strides=(1, 1),  activation='relu',use_bias=False))
#model.add(tf.keras.layers.MaxPooling2D(pool_size=(4, 4)))
model.add(tf.keras.layers.Flatten())
model.add(tf.keras.layers.Dense(4, activation='relu',use_bias=False))
...
if training:
	...
	history = model.fit(data, labels, epochs=100, batch_size=global_batch_size)
	model.save_weights('./saved_models/cnn02')
	print("done training")
else:
	sess = tf.InteractiveSession()
	model.load_weights('./saved_models/cnn02')
	print("done loading")
